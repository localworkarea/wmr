(() => {
    ...
    if (tikers.length > 0) tikers.forEach((tiker => {
        const originalLine = tiker.querySelector(".tiker__list");
        const speed = tiker.getAttribute("data-tiker-speed") || 40;
        if (originalLine) {
            if (tiker.classList.contains("tiker-01")) originalLine.style.animation = `scroll ${speed}s linear infinite`;
            if (tiker.classList.contains("tiker-02")) originalLine.style.animation = `scroll-rev ${speed}s linear infinite`;
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (items.length > 0) if (countServicesElement) countServicesElement.textContent = items.length;
        const closeButtons = document.querySelectorAll(".success__close");
        if (closeButtons.length) closeButtons.forEach((button => {
            button.addEventListener("click", (() => {
                const form = button.closest("form");
                if (form && form.classList.contains("_sent-success")) form.classList.remove("_sent-success");
            }));
        }));
    }));
    window["FLS"] = false;
